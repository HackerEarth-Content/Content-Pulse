...
async def two_members():
    ...
    async with Session() as db:
        out = {}
        for name, email in (
            ("RBAC Ada", "ada@example.com"),
            ("RBAC Grace", "grace@example.com"),
        ):
...
async def test_member_cannot_read_another_members_entries(as_ada, task_type):
    ...
    listed = (
        await c.get(
            "/api/entries",
            params={"from": DAY, "to": DAY, "member_id": ids["RBAC Grace"]},
        )
    ).json()
    ...
    assert (
        await c.get(
            "/api/entries/plan", params={"member_id": ids["RBAC Grace"], "on": DAY}
        )
    ).status_code == 404
...
async def test_member_cannot_file_work_as_someone_else(as_ada, task_type):
    ...
    r = await c.post(
        ...
        json={
            ...
            "entry_date": DAY,
            "items": [
                {"task_type_id": task_type, "notes": "not mine to file", "due_at": DAY}
            ],
        },
    )
...
async def test_admin_routes_refuse_ordinary_members(as_ada):
    c, ids = as_ada
    assert (
        await c.post("/api/members", json={"display_name": "Sneaky"})
    ).status_code == 403
    assert (
        await c.patch(f"/api/members/{ids['RBAC Grace']}", json={"role": "admin"})
    ).status_code == 403
    assert (
        await c.post("/api/meta/lookups/task-types", json={"name": "Sneaky type"})
    ).status_code == 403
...
async def test_ordinary_member_cannot_delete_even_their_own_ticket(as_ada, task_type):
    ...
    r = await c.post(
        "/api/entries/plans",
        json={
            "member_id": ids["RBAC Ada"],
            "entry_date": DAY,
            "items": [
                {"task_type_id": task_type, "notes": "mine to delete?", "due_at": DAY}
            ],
        },
    )
...
async def test_superadmin_email_works_without_a_member_row(monkeypatch):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://t") as c:
        ...
        r = await c.post(
            "/api/meta/lookups/task-types", json={"name": "Bootstrap check"}
        )
