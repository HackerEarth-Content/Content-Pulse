...
async def dataset(client, member, task_type):
    ...
    await client.patch(
        f"/api/entry-items/{plan['items'][0]['id']}", json={"status": "in_progress"}
    )
    await client.post(
        "/api/entries/updates",
        json={
...
async def test_plan_adherence_splits_reported_from_silent(
...
async def test_plan_daily_status_ignores_backfilled_jira_plans(
    ...
    async with Session() as db:
        db.add(
            DailyEntry(
                ...
                kind="plan",
                source="jira",
                idempotency_key=f"jira:{member}:{other_day}",
            )
        )
        await db.commit()
    ...
    rows = (
        await client.get(
            "/api/analytics/plan-daily-status",
            params={"from": DAY, "to": other_day, "member_id": member},
        )
    ).json()
    by_date = {r["entry_date"]: r for r in rows}
    assert by_date[DAY]["planned"] is True
    assert by_date[DAY]["updated"] is True
    assert by_date[other_day]["planned"] is False, (
        "a backfilled Jira day is not a filed plan"
    )
...
async def test_trend_is_zero_filled(client, member, task_type):
    ...
    rows = (
        await client.get(
            "/api/analytics/trend",
            params={"from": "2030-03-01", "to": DAY, "member_id": member},
        )
    ).json()
...
async def test_cycle_time_excludes_work_logged_after_the_fact(
    client, member, task_type, params
):
...
async def test_cycle_time_measures_a_real_interval(client, member, task_type, params):
    ...
    async with Session() as db:
        ...
        first = await db.scalar(
            select(func.min(EntryItemStatusEvent.changed_at)).where(
                EntryItemStatusEvent.entry_item_id.in_(ids)
            )
        )
        await db.execute(
...
async def test_bad_range_rejected(client):
    r = await client.get(
        "/api/analytics/summary", params={"from": "2030-02-01", "to": "2030-01-01"}
    )
...
async def test_effort_breakdown_labels_are_the_real_names(
    client, member, task_type, params
):
    ...
    await client.patch(
        f"/api/entry-items/{plan['items'][0]['id']}", json={"effort_minutes": 60}
    )
...
async def test_assessments_split_out_of_content_requests(
    client, member, task_type, params
):
    ...
    by_area = {
        a["area"]: a
        for a in (await client.get("/api/analytics/by-area", params=params)).json()
    }
    ...
    only = (
        await client.get(
            "/api/analytics/summary", params=params | {"area": "content_assessment"}
        )
    ).json()
