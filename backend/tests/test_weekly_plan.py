...
async def test_status_moves_forward_any_day(client, member, as_monday):
    ...
    r = await client.patch(
        f"/api/weekly-plan/items/{created['id']}", json={"status": "blocked"}
    )
...
async def test_status_cannot_be_set_back_to_yet_to_start(client, member, as_monday):
    ...
    r = await client.patch(
        f"/api/weekly-plan/items/{created['id']}", json={"status": "yet_to_start"}
    )
    assert r.status_code == 422
...
async def test_achievement_locked_until_friday(client, member, as_monday):
    ...
    r = await client.patch(
        f"/api/weekly-plan/items/{created['id']}", json={"achievement": "done!"}
    )
    assert r.status_code == 422
...
async def test_member_cannot_edit_someone_elses_item(as_ada, as_monday):  # noqa: F811
    c, ids = as_ada
    async with Session() as db:
        item = WeeklyPlanItem(
            member_id=ids["RBAC Grace"], week_start=MONDAY, action="grace's item"
        )
        db.add(item)
        await db.commit()
        item_id = item.id
    ...
    r = await c.patch(
        f"/api/weekly-plan/items/{item_id}", json={"status": "in_progress"}
    )
    assert r.status_code == 404, "must not reveal or edit another member's row"


async def test_member_cannot_view_someone_elses_week(as_ada, as_monday):  # noqa: F811
    c, ids = as_ada
    r = await c.get(
        "/api/weekly-plan",
        params={
            "week": MONDAY.isoformat(),
            "member_id": ids["RBAC Grace"],
        },
    )
    assert r.status_code == 200
...
async def test_lead_sees_everyones_weekly_plan_without_member_id(
    client,
    two_members,  # noqa: F811
    as_monday,
):
    ...
    async with Session() as db:
        db.add_all(
            [
                WeeklyPlanItem(
                    member_id=two_members["RBAC Ada"],
                    week_start=MONDAY,
                    action="ada's item",
                ),
                WeeklyPlanItem(
                    member_id=two_members["RBAC Grace"],
                    week_start=MONDAY,
                    action="grace's item",
                ),
            ]
...
async def test_completion_requires_a_lead(as_ada):  # noqa: F811
    c, _ = as_ada
